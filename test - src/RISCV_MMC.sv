...
module RISCV_MMC(
    ...
    // JALR: intermediate named signal required because Vivado cannot bit-slice
    // an expression directly (e.g. (rd1+ext_imm)[31:1] is not legal syntax).
    ...
    PC_Logic pc_logic_inst (
        ...
        .PC_src(pc_src)         // Output decision (2-bit)
    );	
    ...
    // ALUSrcA MUX — selects the ALU's first operand.
    // 2'b01 (LUI)  : src_a = 0         → result = 0 + ext_imm = ext_imm  //Justin
    // 2'b10 (auipc): src_a = PC         → result = PC + ext_imm           //Justin
    // default      : src_a = rd1        → normal register ops
    ...
    // pc_plus4: return address used by both JAL (pcs==2'b10) and JALR (pcs==2'b11)
    assign pc_plus4 = pc_current + 4;

    // Result/Write-back Mux (3-way)
    // Priority: memory load → JAL/JALR link → ALU result
    // JALR (pcs==2'b11): must write PC+4 into rd as the return address,
    // exactly as JAL does. Both cases select pc_plus4.
    assign write_data = (mem_to_reg)                    ? mem_read_data :
                        (pcs == 2'b10 || pcs == 2'b11)  ? pc_plus4      :
                                                          alu_result;

    // Raw JALR target (rs1 + imm), named so Vivado can legally slice it.
    assign jalr_target = rd1 + ext_imm;

    // PC Next Mux (3-way)
    // 2'b01 → taken branch or JAL  : PC + ext_imm
    // 2'b11 → JALR                 : (rs1 + imm) with bit 0 forced to 0
    //                                 (RISC-V spec: result must be 2-byte aligned)
    // default → normal fetch       : PC + 4
    assign pc_next = (pc_src == 2'b01) ? (pc_current + ext_imm)    :
                     (pc_src == 2'b11) ? {jalr_target[31:1], 1'b0} :
                                         (pc_current + 4);
